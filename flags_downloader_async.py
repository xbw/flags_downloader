...
async def main_async():
    """异步主程序"""
    parser = argparse.ArgumentParser(description='国旗图片异步批量下载程序')
    ...
    if not os.path.exists(args.file):
        ...
        if possible_files:
            ...
            print(f"请指定正确的文件名，例如: python flags-downloader-async.py {possible_files[0]}")
            return
        else:
    ...
    print(f"总执行时间: {end_time - time.time():.2f} 秒")

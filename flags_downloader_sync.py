...
def validate_size(size: str) -> bool:
    ...
    if size not in VALID_SIZES:
        ...
        print(f"支持的尺寸选项: {', '.join(VALID_SIZES)}")
        return False
    return True
...
def main():
    """主程序"""
    parser = argparse.ArgumentParser(description='国旗图片同步批量下载程序（多线程版本）')
    ...
    if not os.path.exists(args.file):
        ...
        if possible_files:
            ...
            print(f"请指定正确的文件名，例如: python flags-downloader-sync.py {possible_files[0]}")
            return 0
        else:
    ...
    estimated_time = len(tasks) * 0.5 / args.workers
    if args.filename_format == "full":
        estimated_time *= 1.1
    if args.include_country_name:
        estimated_time *= 1.05
    ...
    if all_results['failed']:
        ...
        with open(failed_file, 'w', encoding='utf-8') as f:
            ...
            f.write(f"文件名格式: {args.filename_format}\n")
            f.write(f"包含国家名称: {'是' if args.include_country_name else '否'}\n")
            f.write("=" * 50 + "\n")
            for country_code, error_msg in all_results['failed']:
    ...
    print(f"\n文件命名格式示例:")
    if args.filename_format == "simple":
        if args.include_country_name:
